...
const api = {
    ...
    async patch(endpoint, body) {
        try {
            ...
            const headers = { 'Content-Type': 'application/json' };
            if (token) headers['Authorization'] = `Bearer ${token}`;
            const response = await fetch(`${API_BASE_URL}${endpoint}`, { method: 'PATCH', headers, body: JSON.stringify(body) });
            const data = await response.json();
            if (!response.ok) throw data;
            return data;
        } catch (error) {
...
const utils = {
    ...
    formatUnit(unit) {
        const units = { 'kg': 'Kilogramos', 'unidades': 'Unidades', 'litros': 'Litros', 'paquetes': 'Paquetes' };
        return units[unit] || unit;
    },

    getBadgeClass(estado) {
        const classes = { 'pendiente': 'badge-pendiente', 'aceptada': 'badge-aceptada', 'rechazada': 'badge-rechazada', 'enviada': 'badge-enviada', 'recibida': 'badge-recibida' };
